...
def calculate_grades():
    ...
    students = User.query.filter(User.grade.isnot(None)).all()

    if not students:
        return "No student grades available to calculate."
    ...
    return f"Average Grade: {average_grade:.2f}<br>Top Student: {top_student.name} with Grade: {top_student.grade}"
